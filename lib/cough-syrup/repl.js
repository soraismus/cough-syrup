...
(function() {
  var CoughSyrup, addHistory, addMultilineHandler, fs, merge, nodeREPL, path, replDefaults, updateSyntaxError, vm, _ref;
  ...
  replDefaults = {
    prompt: 'cough> ',
    historyFile: process.env.HOME ? path.join(process.env.HOME, '.cough_history') : void 0,
  ...
  module.exports = {
    start: function(opts) {
      ...
      if (major === 0 && minor < 8) {
        console.warn("Node 0.8.0+ required for CoughSyrup REPL");
        process.exit(1);
      }
      CoughSyrup.register();
      process.argv = ['cough'].concat(process.argv.slice(2));
